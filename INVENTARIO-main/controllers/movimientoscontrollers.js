...
export const putActivarInactivar = async (req, res) => {
    try {
        const movimiento = await movimientosModel.findById(req.params.id);
        if (!movimiento) {
            return res.status(404).json({ message: 'Movimiento no encontrado' });
        }
        
        movimiento.estado = movimiento.estado === '1' ? '0' : '1';
        await movimiento.save({ validateBeforeSave: false }); // Omitir validación
